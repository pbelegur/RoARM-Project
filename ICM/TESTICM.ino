...
// =====================================================
// 4x ICM20948 Quaternions (Guaranteed full-row output)
//   Bus A (SDA=41,SCL=40): SparkFun DMP Quat6 for 0x68 & 0x69
//   Bus B (SDA=7, SCL=6 ): Raw accel/gyro + Madgwick IMU for 0x68 & 0x69
//
// Output: ONE CSV ROW per tick containing ALL 4 sensors.
// =====================================================
...
// =====================================================
// Helpers: Quat storage + printing (single row)
// =====================================================
struct Quat {
...
Quat qA68, qA69, qB68, qB69;

static void setQuat(Quat &q, float w, float x, float y, float z) {
  q.w = w; q.x = x; q.y = y; q.z = z; q.valid = true;
}

static void printQuatFields(const char *name, const Quat &q) {
  // name,w,x,y,z (wxyz as floats)
  Serial.print(name); Serial.print(",");
  Serial.print(q.valid ? q.w : NAN, 6); Serial.print(",");
  Serial.print(q.valid ? q.x : NAN, 6); Serial.print(",");
  Serial.print(q.valid ? q.y : NAN, 6); Serial.print(",");
  Serial.print(q.valid ? q.z : NAN, 6);
}

static void printAllRow() {
  // 4 lines (one per sensor), then a separator line
  printQuatFields(NAME_A1, qA68); Serial.println();
  printQuatFields(NAME_A2, qA69); Serial.println();
  printQuatFields(NAME_B1, qB68); Serial.println();
  printQuatFields(NAME_B2, qB69); Serial.println();
  Serial.println("---------");
}


// =====================================================
// DMP init + update (Bus A)
// =====================================================
...
static bool updateDMPQuat(ICM_20948_I2C &imu, icm_20948_DMP_data_t &d, Quat &out) {
...
void setup() {
  Serial.begin(921600);   // IMPORTANT: set Serial Monitor to 921600
  delay(1500);

  Serial.println("\n--- 4x ICM20948 Quaternions (Full-row output) ---");
  Serial.println("Bus A (41/40): DMP Quat6 for 0x68 and 0x69");
  Serial.println("Bus B (7/6):   Raw accel/gyro + Madgwick for 0x68 and 0x69");
  Serial.println("Each row prints ALL 4 sensors:");
  Serial.println("A_41_40_68,w,x,y,z,A_41_40_69,w,x,y,z,B_7_6_68,w,x,y,z,B_7_6_69,w,x,y,z\n");
...
void loop() {
  // 1) Update Bus A DMP (grab latest when available)
  if (okA68) (void)updateDMPQuat(imuA1, dmpDataA1, qA68);
  if (okA69) (void)updateDMPQuat(imuA2, dmpDataA2, qA69);

  // 2) Update Bus B Madgwick for each sensor ~100Hz each
  if (okB68) {
    uint32_t now = micros();
    ...
    if (dt >= 0.01f) { // 100 Hz
      ...
      if (readAccelGyro_raw(BUS_B, ADDR_68, ax,ay,az,gx,gy,gz)) {
        ...
        setQuat(qB68, filtB68.q0, filtB68.q1, filtB68.q2, filtB68.q3);
      }
    }
  }
  ...
  if (okB69) {
    ...
    if (dt >= 0.01f) { // 100 Hz
      ...
      if (readAccelGyro_raw(BUS_B, ADDR_69, ax,ay,az,gx,gy,gz)) {
        ...
        setQuat(qB69, filtB69.q0, filtB69.q1, filtB69.q2, filtB69.q3);
      }
    }
  }

  // 3) Print at fixed rate so Serial never chokes
  static uint32_t lastPrintMs = 0;
  const uint32_t PRINT_HZ = 50;               // change to 100 if you want
  const uint32_t PRINT_PERIOD_MS = 1000 / PRINT_HZ;

  uint32_t nowMs = millis();
  if (nowMs - lastPrintMs >= PRINT_PERIOD_MS) {
    lastPrintMs = nowMs;
    printAllRow();                             // ✅ always prints all 4
  }
}
